...
namespace DGL {
	...
	// Shader::Stage::Unit
		...
		void Shader::Stage::Unit::Init()
		{
			...
			{// Load the texture
				...
				Texture::Filter filter( Texture::Filter::TRILINEAR, edgeClamp);
	...
	// Shader Library
		...
		const Shader *ShaderLib::GetShaderForTexture( const char *texName)
		{
			for( int i=0; i< this->currentFree; i++)
			{
				if( !strcmpi( texName, this->shaders[i].name) )
				{
		...
		void ShaderLib::executeCommand( const char * command, const char * param, Shader::Stage::Unit *unit)
		{
			...
			if ( !strcmpi( command, "SetMap") )				{ unit->SetMap(param); return;}
			if ( !strcmpi( command, "AddFlag") )			{ unit->AddFlag(param); return;}
			if ( !strcmpi( command, "AlphaFactor") )		{ unit->AlphaFactor(atof(param)); return;}
			if ( !strcmpi( command, "TexgenFactorS") )		{ unit->TexgenFactorS(atof(param)); return;}
			if ( !strcmpi( command, "TexgenFactorT") )		{ unit->TexgenFactorT(atof(param)); return;}
			if ( !strcmpi( command, "TexgenFactorRotate") )	{ unit->TexgenFactorRotate(atof(param)); return;}
