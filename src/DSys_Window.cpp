...
------------
	...
	$Id: DSys_Window.cpp,v 1.3 2004/07/31 19:22:02 Daher Alfawares Exp $
	$Name:  $
...
namespace DSys {
	...
	Var_bool		sv_bFullscreen			("sys_fullscreen",		"0");
	Var_int			sv_iScreenWidth			("sys_screenWidth",		"640");
	Var_int			sv_iScreenHeight		("sys_screenHeight",	"480");
	Var_int			sv_iColorBits			("sys_colorBits",		"32");
	Var_int			sv_iDepthBits			("sys_depthBits",		"24");
	Var_bool		sv_bFixMousePosition	("sys_fixMousePosition","1");

	Var_string		sv_sTitle				("sys_title",			"Daher Engine", Var::NoSet);
};
...
int				DSys::Window::width;
int				DSys::Window::height;
...
//		case WM_SIZE:				Window::Resize(LOWORD(lParam),HIWORD(lParam)); break;
	}
...
void DSys::Window::Destroy(void){
	...
	if (!UnregisterClass("DaherEngine",hInstance))				// Are We Able To Unregister Class
	{
...
bool DSys::Window::Create( DSys::Window::Info* info ){
	...
	DSys::Window::isFullscreen	= info->fullscreen;
	DSys::Window::width			= info->width;
	DSys::Window::height		= info->height;
	...
	wc.lpszClassName= "DaherEngine";						// Set The Class Name
	...
	static	PIXELFORMATDESCRIPTOR pfd=						// pfd Tells Windows How We Want Things To Be
	{
		sizeof(PIXELFORMATDESCRIPTOR),						// Size Of This Pixel Format Descriptor
			...
			info->alphaBits,									// No Alpha Buffer
			0,													// Shift Bit Ignored
			info->accumBits,									// No Accumulation Buffer
			0, 0, 0, 0,											// Accumulation Bits Ignored
			info->depthBits,									// Z-Buffer (Depth Buffer)
			info->stencilBits,									// Stencil Buffer
			info->auxBuffers,									// Auxiliary Buffer
...
bool DSys::Window::ChangeRes(int width, int height, int bits){
	...
	switch(result){
		...
		default:
			DSys::Logger::Print("Falling back to defaults, 640x480");
			DMacro_TraceLeave();
			return ChangeRes(640,480,32);	
	}
}
