...
------------
	...
	$Id: DSys_Console.cpp,v 1.3 2004/07/31 19:22:02 Daher Alfawares Exp $
	$Name:  $
...
#define CON_MAX_Y				480
#define CON_DST_Y				240
#define CON_SPEED				6
...
void DSys::Console::GFX_Render(float dt){
			...
			conback.Set(
				...
				0.5f);
			DGL::Draw2DRect(0,y,640, CON_MAX_Y-y,conback);
			DGL::Draw2DRect(0, y, 640, CON_MAX_Y-y,gfx.back1);
			DGL::Draw2DRect(0,y,640,CON_MAX_Y-y,gfx.back2,
				0.0f+TexSpeed, 0.0f,
				4.0f+TexSpeed, 0.0f,
				4.0f+TexSpeed, 2.0f,
				0.0f+TexSpeed, 2.0f);
			DGL::Draw2DRect(0,y,640,CON_MAX_Y-y,gfx.back3,
			...
			DGL::Draw2DRect(
				...
				gfx.logo);
#else
			DGL::Draw2DRect(0,y,640, CON_MAX_Y-y, DGL::Color(0,0,0,0.35f));
#endif //0
			DGL::Draw2DLine(0,y,640,y,2.0f,DGL::Color::ColorRed());

			if(lines_up)
				DGL::Draw2DLine(0,y+27.5,640,y+27.5,0.1f,DGL::Color::ColorBlack());
			...
			gfx.font.Print(638, y+8, DSys::Version);
...
//video Options
	...
	void vid_setMode(char *param)
	{
		...
		else 
		{
			if(!DSys::Window::ChangeRes(vid_ModeList[mode].w, vid_ModeList[mode].h, DSys::sv_iColorBits.intval()))
			{
				...
				DSys::sv_iScreenHeight.setval( "640" );
				DSys::sv_iScreenWidth.setval( "480" );
				DSys::sv_iScreenHeight.update();
				DSys::sv_iScreenWidth.update();
				return;
			}
	...
	{
		DGL::Texture::_ScreenShot(DSys::Window::Width(), DSys::Window::Height());
	}

}//End namespace DSys	

/* ------------------------------------ */
/*  DSys::* console related methods  */
/* ------------------------------------ */
