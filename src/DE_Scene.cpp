...
------------
	...
	$Id: DE_Scene.cpp,v 1.8 2004/07/31 19:21:59 Daher Alfawares Exp $
	$Name:  $
...
/**/
	CHECK_QUIT;
	g_loadingScreen.SetStage(75, "Initializing Lighting Scene ...");
...
void Update(float msec){
	...
	if(SceneMode == MODE_PLAYBACK) {
		...
		if(DSys::Input::KeyDown(DIK_LALT) || DSys::Input::KeyDown(DIK_RALT)){
			...
			/////////////
			// Camera movement
			AT_KEY_DOWN(DIK_W)		g_camera.MoveCameraFarward((DSys::Input::KeyValue(DIK_W)/1000.0f) * g_speed.floatval() );
			AT_KEY_DOWN(DIK_S)		g_camera.MoveCameraFarward(-(DSys::Input::KeyValue(DIK_S)/1000.0f) * g_speed.floatval() );
			AT_KEY_DOWN(DIK_D)		g_camera.MovePlaneRight((DSys::Input::KeyValue(DIK_D)/1000.0f) * g_speed.floatval() );
			AT_KEY_DOWN(DIK_A)		g_camera.MovePlaneRight(-(DSys::Input::KeyValue(DIK_A)/1000.0f) * g_speed.floatval() );
			AT_KEY_DOWN(DIK_SPACE)	g_camera.MovePlaneUp((DSys::Input::KeyValue(DIK_SPACE)/1000.0f) * g_speed.floatval() );
			AT_KEY_DOWN(DIK_C)		g_camera.MovePlaneUp(-(DSys::Input::KeyValue(DIK_C)/1000.0f) * g_speed.floatval() );
...
void Render3D(){
	...
	if(SceneMode != MODE_PLAYBACK){
		...
		glClearColor( 0,0,0,1);
/*		glBindTexture(GL_TEXTURE_2D, g_refMap);
		glEnable(GL_TEXTURE_GEN_S);
		glEnable(GL_TEXTURE_GEN_T);
			glTranslatef(0,0,-7.5f);
			glRotatef(20,1,0,0);
			glColor4f(1,1,1,1);
			glRotatef(-g_timeCounter, 0, 1, 0);
			g_font3D.Print("Horror");
		glDisable(GL_TEXTURE_GEN_S);
		glDisable(GL_TEXTURE_GEN_T);
  */
		glPushAttrib(GL_LIGHTING_BIT);
		glDisable(GL_LIGHTING);
		DGL::Draw2DRect( 320-70, 300, 140, 140, g_menuBack);
		glPopAttrib();
	}
...
//==============================================================
		...
		if(r_drawTimer.intval()){
			g_font.SetStyle(g_font.STYLE_SHADOWS|g_font.STYLE_RIGHT);
			g_font.SetColor(DGL::Color::ColorGray());
			g_font.Print(635, 445, "%s", D_Msec2String(totaltime));
		}
