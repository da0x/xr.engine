...
------------
	...
	$Id: DE_BouncingBalls.h,v 1.3 2004/07/31 19:21:59 Daher Alfawares Exp $
	$Name:  $
...
namespace DBouncingBalls {
		...
		class Arena {
			public:
				const static MinX = 20;
				const static MaxX = 600;
				const static MinY = 40;
				const static MaxY = 450;
				const static BorderSize = 2;
	...
	//				DGL::Draw2DLine(MaxX,MinY,MinX,MinY,BorderSize,DGL::Color::ColorGray());
		...
		class MessileBall : public Ball {
			private:
				const static	radius = 8;
				const static	mass = 10;
	...
	class Game {
		private:
				const static		targetMass	= 15;
				const static		addTime		= 1000;
				const static		dieDelay	= 2000;
				const static		MaxLevels	= 7;
				const static		hitLevels[MaxLevels];
				const static		targetRadius[MaxLevels];
	...
	const Game::hitLevels[MaxLevels] = {
		0, 10, 25, 50, 75, 100, 125
	};
	const Game::targetRadius[MaxLevels] = {
		30, 25, 20, 15, 10, 5, 3
	};
}
